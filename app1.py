...
if st.sidebar.button("Predict Closing Price"):
    ...
    try:
        ...
        end_date = selected_date_obj.strftime('%Y-%m-%d')
        start_date = (selected_date_obj - timedelta(days=5)).strftime('%Y-%m-%d')
        ...
        # Filter to dates <= selected date
        hist = hist[hist.index <= selected_date_obj]
        ...
        if not hist.empty:
            ...
            compare_df = pd.DataFrame({
                'Price': [actual_close, prediction]
            }, index=['Actual', 'Predicted'])

            st.bar_chart(compare_df)

        else:
            st.warning("No actual data found within range. Stock may not have traded during this time.")
